...
import net.co5mowulf.mccourse.features.ModUndergroundConfiguredFeatures;
...
public class GlowMossBlock extends Block implements Fertilizable {
    ...
    public void grow(ServerWorld world, Random random, BlockPos pos, BlockState state) {
        world.getRegistryManager().getOptional(RegistryKeys.CONFIGURED_FEATURE).flatMap((registry) -> {
            return registry.getEntry(ModUndergroundConfiguredFeatures.GLOW_MOSS_PATCH_BONEMEAL);
        }).ifPresent((reference) -> {
